...
const correctAnswer = "partai persatuan";
...
form.addEventListener("submit", function(event) {
  ...
  if (guess === correctAnswer || guess === correctAnswer.trim()) {
    ...
    setTimeout(function() {
      window.location.href = "selector.html";
      localStorage.setItem('level17Complete', 'true');
    }, 2000);
  } else {
...
document.getElementById("hint-btn").addEventListener("click", function() {
  swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      swal.fire({
        title: "Hint",
        text: "Pada masa orde lama, partai-partai yang berbasis Islam tergabung dalam.",
        icon: "info",
        button: "OK"
      });
